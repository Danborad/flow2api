...
class WebhookService:
    ...
    async def send_wecom_markdown(self, content: str, custom_url: Optional[str] = None) -> tuple[bool, str]:
        ...
        if not url:
            ...
            url = (cfg.wecom_webhook_url or "").strip()
        ...
        # 企业微信 markdown 长度限制 4096 字节
        encoded = content.encode("utf-8")
        if len(encoded) > 4000:
            content = encoded[:3950].decode("utf-8", errors="ignore") + "\n\n...(内容过长已截断)"
    ...
    async def send_test_message(self, custom_url: Optional[str] = None) -> tuple[bool, str]:
        """发送测试消息"""
...
如果您收到了此条消息，说明 Flow2API 的企业微信 Webhook 配置正确，已可正常接收实时账号告警与每日统计汇报！"""
        return await self.send_wecom_markdown(markdown, custom_url=custom_url)
...
💡 **处理建议**：请在对应电脑浏览器中打开 Flow（`https://flow.google.com/`）确认登录状态，并在插件中点击“导入当前 Google 账号”进行恢复。"""
            ...
            success, msg = await self.send_wecom_markdown(markdown)
            if success:
...
{account_section}{abnormal_section}"""
    ...
    async def send_daily_report(self, custom_url: Optional[str] = None) -> tuple[bool, str]:
        """构建并发送每日生成与账号汇报"""
        try:
            report_text = await self.build_daily_report()
            return await self.send_wecom_markdown(report_text, custom_url=custom_url)
        except Exception as e:
