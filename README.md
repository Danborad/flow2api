...
管理后台“系统配置 -> 生成配置”中的“图片失败兜底次数”控制账号切换重试：
...
兜底只适用于图片生成，不会重复使用已经尝试过的账号。提示词安全策略拒绝、参数错误等请求级失败也会按配置尝试其他账号，但不会因此自动禁用原账号。

每次实际切换账号都会单独写入一条“图片兜底”请求日志，主日志保留整次请求的最终结果，不会被备用账号尝试覆盖。
...
- **管理后台配置**：“系统配置 -> 企业微信通知配置”中可一键配置 Webhook 地址、测试通知、或点击“立即汇报”即刻生成并发送报告。
