...
当前扩展版本会显示在插件设置页标题中，例如 `v1.1.28`。Chrome 扩展管理页的“详情”中也会显示同一版本；后台 WebSocket 日志会记录连接插件的版本号。设置页还会显示 WebSocket 当前连接状态、最近错误、插件事件日志以及导入凭据的过期时间。服务地址权限在点击保存时由 Chrome 临时申请。
